...
class ServicoSerializer(serializers.ModelSerializer):
    class Meta:
        model = Servico
        fields = ['id', 'nome', 'descricao', 'preco_sugerido', 'ativo']
...
class TipoVeiculoSerializer(serializers.ModelSerializer):
    class Meta:
        model = TipoVeiculo
        fields = ['id', 'nome', 'descricao', 'ativo', 'data_criacao', 'usuario']
        read_only_fields = ['data_criacao']

class CategoriaChecklistSerializer(serializers.ModelSerializer):
    class Meta:
        model = CategoriaChecklist
        fields = ['id', 'nome', 'descricao', 'ordem', 'ativo', 'data_criacao', 'usuario']
        read_only_fields = ['data_criacao']
...
class ItemChecklistSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            ...
            'tipo_verificacao', 'critico', 'ordem', 'ativo', 'data_criacao',
            'usuario', 'obrigatorio'
        ]
        read_only_fields = ['data_criacao']
...
class ItemChecklistPersonalizadoSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        fields = [
            'id', 'checklist', 'checklist_nome', 'item_padrao', 'item_padrao_nome',
            'nome', 'descricao', 'categoria', 'categoria_nome', 'tipo_verificacao',
            'critico', 'obrigatorio', 'ordem', 'ativo', 'data_criacao'
        ]
        read_only_fields = ['data_criacao']
...
class ChecklistExecutadoSerializer(serializers.ModelSerializer):
    ...
    class Meta:
        ...
        read_only_fields = ['data_execucao', 'data_atualizacao']
...
class ChecklistExecutadoDetalhadoSerializer(serializers.ModelSerializer):
    ...
    itens_executados = ItemChecklistExecutadoSerializer(many=True, read_only=True)
    
    class Meta:
        model = ChecklistExecutado
        fields = [
            'id', 'checklist', 'veiculo', 'data_execucao', 'usuario',
            'observacoes', 'data_atualizacao', 'status', 'itens_executados'
        ] 